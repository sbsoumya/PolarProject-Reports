...
interest.\footnote{This project is supported by Robert Bosch Center for Cyber Physical Systems}
...
%----------Paper intro
Random correlated data (X,Y) is distributed between two parties with the first observing X and the second observing Y. The two parties seek to recover each others data. The \emph{Data-Exchange} problem depicted in figure \ref{fig:dataex}, encompasses this scenario. This project seeks to device a practical protocol which achieves this with minimal communication under a setting where the joint distribution of X and Y is unknown.\\
...
\begin{wrapfigure}{r}{0.4\textwidth}
  ...
  \label{fig:swcomp}
\end{wrapfigure}
\paragraph{Slepian-Wolf compression:}As described in \cite{discus}, consider first the problem where X and Y are correlated discrete-alphabet memoryless sources, we have to compress X losslessly, with Y (side information) being known at the decoder and \emph{not} at the encoder. If Y were known at both ends one can compress X at a theoretical rate of $H(X|Y)$. But if Y were known only at decoder the same can be achieved by just knowing $P_{X|Y}$ at encoder without explicit information of Y, this has been depicted in figure \ref{fig:swcomp}. Thus for communicating X to the second party a rate of  $H(X|Y)$ is sufficient.
\clearpage
...
In this project we circumvent this difficulty by implementing Slepian-Wolf compression using structured channel codes such as Polar Codes as shown in \cite{discus},\cite{pslep}. We use the \emph{recursive data exchange} (RDE) protocol  mentioned in \cite{htsw} for the case the where underlying joint distribution is unknown. Note that in \cite{htsw} the decoders suggested for the protocol are theoretical constructs and are not practically implementable. We use Rateless Polar Codes for the implementation of RDE. 
...
\end{itemize}
Interestingly, this approach is closely related to the construction of an Hybrid ARQ with error detection in the PHY layer.
\\In section \ref{bg} we provide the background required to build our proposal.  Section \ref{propsol}, consolidates and elaborates the proposed scheme. Performance analysis of the scheme is presented in section \ref{perfana}. We conclude with final remark and future work in section \ref{future}.
%----------------------------------------------------------------------------------------
%	Background
%----------------------------------------------------------------------------------------
\clearpage
\section{Background}\label{bg}
In this section we discuss the the artifacts needed for the suggested implementation in brief.
...
\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=1\textwidth]{fer.png}
  \end{center}
...
As indicated in section \ref{intro}, our approach towards implementation of a solution to the \emph{Data-Exchange} problem using RDE and Polar Codes can be summarised as below, 
\begin{itemize}
...
\clearpage
\section{Performance Evaluation}\label{perfana}
%-------------------------------SW
